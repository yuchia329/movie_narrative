...
log = logging.getLogger("jieshuoforge_web.metrics")
    ...
    def collect(self):
        ...
        try:
            ...
            runs = GaugeMetricFamily(
                "jieshuo_runs", "Run count by status (snapshot)", labels=["status"]
            )
            inprog = 0
            ...
            yield runs
            g = GaugeMetricFamily("jieshuo_runs_in_progress", "Queued + running runs")
            g.add_metric([], inprog)
            yield g
            mt = GaugeMetricFamily("jieshuo_movies_total", "Registered movies")
            mt.add_metric([], movies_total)
            yield mt
            st = GaugeMetricFamily("jieshuo_active_sessions_total", "Sessions with runs")
        ...
        try:
            ...
            b = Budget()
            for name, val, doc in (
                ("jieshuo_llm_budget_remaining_usd", b.remaining_usd(), "USD of LLM credit left"),
                ("jieshuo_llm_cost_usd_total", b.spent_usd(), "USD of LLM credit spent"),
                ("jieshuo_llm_budget_reserved_usd", b.reserved_usd(), "USD held by in-flight runs"),
                ("jieshuo_llm_budget_cap_usd", get_settings().llm_max_cost_usd, "Configured spend cap"),
            ):
        ...
        try:
            ...
            qd = GaugeMetricFamily("jieshuo_queue_depth", "Pending tasks per queue", labels=["queue"])
        ...
        try:
            ...
            savg = GaugeMetricFamily(
                "jieshuo_stage_seconds_avg", "Mean per-stage wall time (recent)",
                labels=["scope", "stage", "status"],
            )
            smax = GaugeMetricFamily(
                "jieshuo_stage_seconds_max", "Max per-stage wall time (recent)",
                labels=["scope", "stage", "status"],
            )
            srun = GaugeMetricFamily(
                "jieshuo_stage_running", "Stages currently in the running state",
                labels=["scope", "stage"],
            )
        ...
        try:
            ...
            cost = GaugeMetricFamily(
                "jieshuo_llm_stage_cost_usd_total", "Cumulative LLM USD by pipeline stage",
                labels=["stage"],
            )
            toks = GaugeMetricFamily(
                "jieshuo_llm_stage_tokens_total", "Cumulative LLM tokens by pipeline stage",
...
def push_run_metrics(
    ...
    *,
    ...
    try:
        ...
        dur = Histogram(
            "jieshuo_stage_duration_seconds", "Per-stage wall time",
            ["stage", "lang", "status"],
        ...
        Counter("jieshuo_llm_tokens", "LLM tokens this run", ["lang", "direction"], registry=reg)\
            .labels(lang, "in").inc(tokens_in)
        Counter("jieshuo_llm_tokens_out_run", "", ["lang"], registry=reg).labels(lang).inc(tokens_out)
        Gauge("jieshuo_run_llm_cost_usd", "LLM USD this run", ["lang"], registry=reg).labels(lang).set(cost_usd)
        if json_retries:
            Counter("jieshuo_llm_json_retries", "Non-JSON LLM retries", ["lang"], registry=reg)\
                .labels(lang).inc(json_retries)

        # output-quality proxies
        if output_duration_sec is not None:
            Gauge("jieshuo_output_duration_seconds", "Final recap length", ["lang"], registry=reg)\
                .labels(lang).set(output_duration_sec)
        if narration_lines is not None:
            Gauge("jieshuo_script_lines", "Script lines by kind", ["lang", "kind"], registry=reg)\
                .labels(lang, "narration").set(narration_lines)
        if playback_lines is not None:
            Gauge("jieshuo_script_lines", "Script lines by kind", ["lang", "kind"], registry=reg)\
                .labels(lang, "playback").set(playback_lines)
        if failed_stage:
            Counter("jieshuo_stage_failures", "Stage failures", ["stage", "lang"], registry=reg)\
                .labels(failed_stage, lang).inc()

        push_to_gateway(gw, job="jieshuo_run", grouping_key={"run_id": run_id}, registry=reg)
