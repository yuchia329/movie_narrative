...
from jieshuoforge import pipeline
from jieshuoforge.config import Config, load_config
from jieshuoforge.storage import Storage
from jieshuoforge.timing import RunTimer
...
log = logging.getLogger("jieshuoforge_web.tasks")
S = get_settings()

app = Celery("jieshuoforge", broker=S.broker_url, backend=S.result_backend)
app.conf.update(
    ...
    task_routes={
        "jieshuoforge_web.tasks.fh_local": {"queue": "cpu"},
        ...
        "jieshuoforge_web.tasks.fh_asr": {"queue": "asr"},
        "jieshuoforge_web.tasks.fh_scenes": {"queue": "cpu"},
        "jieshuoforge_web.tasks.mark_ready": {"queue": "cpu"},
        "jieshuoforge_web.tasks.bh_script": {"queue": "llm"},
        "jieshuoforge_web.tasks.bh_tts": {"queue": "tts"},
        "jieshuoforge_web.tasks.bh_render": {"queue": "render"},
        "jieshuoforge_web.tasks.finalize_run": {"queue": "render"},
    },
)
...
def mark_ready(movie_id: str) -> str:
    ...
    with session_scope() as db:
        ...
        if movie is not None and movie.status != MovieStatus.error:
            ...
            try:
                from jieshuoforge.schemas import ProbeManifest
                ...
                pass
    # One-step UX: a language was chosen at upload, so kick the recap off automatically as soon
...
def _build_guarded_client(cfg: Config, guard: BudgetGuard, *, on_usage=None):
    ...
    """
    from jieshuoforge.llm.client import LLMClient
...
def bh_render(self, run_id: str) -> str:
    ...
    try:
        ...
        _persist(store, cfg, movie_path, prefix)
        # upload the final recap to a stable key for presigned download
...
def finalize_run(run_id: str) -> str:
    ...
    out_dur = None
    try:
        from jieshuoforge.schemas import Edl, Script
...
# convenience for `celery -A jieshuoforge_web.tasks worker`
celery_app = app
