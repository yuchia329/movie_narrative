...
def run(
    ...
    lang: str = typer.Option(None, "--lang", help="narration language: zh (Mandarin 解说) | en (English recap); default from config"),
    ...
    try:
        ...
        console.print("set LLM_API_KEY (script) and TTS_GRPC_TARGET (voiceover gRPC), then re-run.")
    console.print(f"per-stage timings written to {timings_path}")
...
def submit(
    movie: str,
    remote: str = typer.Option(..., "--remote", help="platform base URL, e.g. https://recap.example.com"),
    ...
    """Upload a movie to a running platform and generate a recap remotely.

    Convenience wrapper over the HTTP API (presigned upload -> front half -> run). The
    local `jieshuo run` is unaffected; this just drives the same pipeline on the server.
    """
    import time
    ...
    with httpx.Client(timeout=60, follow_redirects=True) as c:
        ...
        reg = c.post(f"{base}/api/movies", json={"filename": path.name}).raise_for_status().json()
        console.print(f"uploading [bold]{path.name}[/bold] → storage…")
        with path.open("rb") as f:
            up = c.request(reg["method"], reg["upload_url"], content=f.read(),
        ...
        while True:
            ...
            if m and m["status"] == "error":
                ...
                raise typer.Exit(1)
            time.sleep(5)
        run = c.post(f"{base}/api/runs", json={"movie_id": mid, "lang": lang}).raise_for_status().json()
        ...
        console.print(f"[green]done[/green] — recap: {res['url']}")
