...
The 解说 script brain runs on MiniMax via its OpenAI-compatible chat endpoint
...
log = logging.getLogger("jieshuoforge.llm")
        ...
        *,
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                messages.append({"role": "user", "content": "上一条不是合法 JSON。请只输出符合 schema 的 JSON 对象。"})

        raise LLMError(f"model did not return valid JSON after {self.max_retries + 1} attempts: {last_err}")
