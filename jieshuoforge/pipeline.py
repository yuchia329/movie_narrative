...
def _asr_via_server(wav: Path, cfg: Config, target: str | None = None) -> Transcript | None:
...
#   "cpu" = local CPU / ffmpeg. Mirrors the Celery queue routing in jieshuoforge_web.
STAGE_COMPUTE = {
    "ingest": "cpu", "audio": "cpu", "asr": "gpu", "shots": "cpu", "scenes": "cpu",
    "understand": "llm", "script": "llm", "budget": "cpu",
    "tts": "gpu", "edl": "cpu", "subs": "cpu", "render": "cpu",
}
...
def resolve_lang(cfg: Config, lang: str | None = None) -> str:
    ...
    if lang not in SUPPORTED_LANGS:
        raise RuntimeError(f"unsupported narration language {lang!r} (expected one of {SUPPORTED_LANGS})")
    return lang
...
def run_back_half(
    ...
    lang: str | None = None, llm_client=None, until: str | None = None, tts_target: str | None = None,
    ...
    def _stop(stage: str) -> bool:
        return until is not None and until in BACK_HALF and BACK_HALF.index(stage) >= BACK_HALF.index(until)
    from . import db
    ...
    # Each language maps to a platform profile (Douyin 解说 vs YouTube recap conventions).
    ...
    log.info("[back] narration language=%s platform=%s target=%ds", lang, platform, target_sec)
    ...
    client = llm_client
    if client is None and (force or not screenplay_path.exists() or not script_path.exists()):
        client = LLMClient(
    ...
    if force or not screenplay_path.exists():
        with timer.stage("understand"):
            screenplay = s06_understand.run_stage(
                conn, client, target_sec=target_sec,
                thinking=lcfg.get("thinking_map", "adaptive"), lang=lang,
            )
    ...
    if force or not script_path.exists():
        with timer.stage("script"):
            script = s07_script.run_stage(
                ...
                thinking=lcfg.get("thinking_reduce", "disabled"), lang=lang,
            )
    ...
    if lang == "zh":
        ...
        if not ref_wav:
            log.warning(
                ...
                "set [tts.%s].reference_clip/reference_text for a %s narrator", lang, lang, lang,
            )
    if force or not vo_path.exists():
        with timer.stage("tts"):
            ...
            targets = [t for t in (tts_targets or ([tts_target] if tts_target else [])) if t]
            if not targets:
                targets = [cfg.require_env("TTS_GRPC_TARGET")]
            synths = [TTSClient(t) for t in targets]
            try:
                vo = s09_tts.run_stage(
                    script_final, synths[0], bdir / "vo",
                ...
                for s in synths:  # release the K gRPC channels (the worker is long-lived)
                    try:
                        s.close()
    ...
    if acfg.get("score_bed", False):
        stem = next((mdir / f"audio_novocals.{ext}" for ext in ("flac", "wav", "opus")
                     if (mdir / f"audio_novocals.{ext}").exists()), None)
        if stem is not None:
            score_stem = str(stem)
        else:
            log.warning(
                "[render] score_bed enabled but no audio_novocals.* at %s — run scripts/separate_score.py; "
                "falling back to ducked original audio under narration", mdir,
            )
    with timer.stage("render"):
        out = s12_render.run_stage(
            movie_path, edl,
            scratch_dir=cfg.scratch_dir / mdir.name / lang, out_path=bdir / "recap_final.mp4",
