#!/usr/bin/env bash
# Build the recap image for linux/arm64 and load it into the k3s containerd on the prod box
...
#
# The k3s kubelet resolves the manifest's `recap:<tag>` to docker.io/library/recap:<tag>,
...
echo "     # or: (cd deploy/k8s/overlays/prod && kustomize edit set image jieshuoforge=${IMAGE}:${IMAGE_TAG})"
